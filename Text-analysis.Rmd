...
Here i will use GloVe to make a word-embedding model. We will do so in a quanteda corpus->token->fcm workflow. To create the mebeddings, we will use the text2vec library.
...
We here use the tidy and cleaned tweet data. 

```{r}
head(tweets_tidy)
tweets_tidy %>% 
  cast_dfm(document = ID, term = tweet, value = count)
```
